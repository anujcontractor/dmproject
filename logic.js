...
var decryptbtn = document.getElementById("print");
var bn;
...
decryptbtn.addEventListener('click', func1);
...
var s = [["d1310ba6", "98dfb5ac", "2ffd72db", "d01adfb7", "b8e1afed", "6a267e96", "ba7c9045", "f12c7f99",
...
function subKeys(){
    var j=0;
    for(let i=0 ; i<18 ; i++){
        p[i] = xor((p[i]),(key.value.substr(j,j+8)));
        //op.innerHTML = "subkey " + (i + 1) + ": " + p[i];
        j = (j+8)%key.value.length;
    }
...
function round(number,hextext){

    var left = hextext.substring(0,8);
    var right = hextext.substring(8,16);

    left = xor(left,p[number]);
    ...
    var z = right + left;
    if(z.length < 16){
        for(var j=0 ; j<= 16 - z.length ; j++){
            z = "0" + z;
        }
    }


    //op.innerHTML = "round " + number + ": " + z;

    return z;
    
}

function encrypt(playtext){
...
function split(plaintext){
    ...
    op.innerHTML = arr[i-1].length;
    if(arr[i-1].length < 16){
        for(var j=0 ; j<= 16 - arr[i-1].length ; j++){
            arr[i-1] = "0" + arr[i-1];
        }
    }
...
function addBin(a,b){
    ...
    var int = temp;
    if(int>0){
        int = (parseInt(int,10)).toString(16);
        int = "00000000" + int;
        return int.substring(int.length - 8);
    }
    else{
        int = (parseInt(int,10)>>>0).toString(16);
        int = "00000000" + int;
        return int.substring(int.length - 8);
    }
...
function funcF(text){
    ...
    for(var i=0 ; i<4 ; i++){
        arr[i] = text.substring(k,k+2);
        k = (k+2)%text.length;
        arr[i] = parseInt(arr[i],16); // To store hex to dec value

        arr[i] = s[i][arr[i]]; // To store S-box value;
    }

    ans = addBin(arr[0], arr[1]);
    ans = xor(ans, arr[2]);
    ans = addBin(ans, arr[3]);
    var z = ans;
    if(z.length < 8){
        for(var j=0 ; j<= 8 - z.length ; j++){
            z = "0" + z;
        }
    }
...
function decrypt(cyphertext){
    subKeys();
    for(var i = 17 ; i>1 ; i--){
        cyphertext = round(i, cyphertext);
    }

    var right = cyphertext.substring(0, 8);
